...
IMGUI_DEF int imgui_list(Vec2f pos, Vec2f size, Vec4f primary, Vec4f secondary, Imgui_ListView *listView) {
  ...
  //height needed :: listView->len * (float) imgui_instance.font->height

  float font_height = (float) imgui_instance.font->height;

  //SCROLL PANE
  float width = 8.f;
  Vec2f pane_size = vec2f(width, size.y * size.y / listView->len / font_height);
  Vec2f pane_pos = vec2f(pos.x + size.x - width,
			 pos.y + size.y - pane_size.y - listView->scroll_pos);
  float scroll_step = (1 + listView->len - size.y / font_height) / (size.y - pane_size.y);
  bool fit_on_screen = size.y < font_height * listView->len;
  if(fit_on_screen) {
    IMGUI_RENDERER_UPDATE_SHADER(SHADER_FOR_COLOR);
    Vec4f color = secondary;
    bool clicked = IMGUI_RENDERER_VEC_IN_RECT(imgui_instance.input, pane_pos, pane_size);
    if(clicked && listView->off < 0.f) {
      listView->off = (pane_size.y + pane_pos.y) - imgui_instance.input.y;
    }	    
    if(listView->off >= 0.f) {
      pane_pos.y = imgui_instance.pos.y - pane_size.y + listView->off;
      color.w *= .5f;
    }
    
    if(pane_pos.y + pane_size.y >= pos.y + size.y) pane_pos.y = pos.y + size.y - pane_size.y;
    if(pane_pos.y < pos.y) pane_pos.y = pos.y;
    listView->scroll_pos = (size.y - pane_size.y) - (pane_pos.y - pos.y);

    bool hovering = IMGUI_RENDERER_VEC_IN_RECT(imgui_instance.pos, pos, size);
    if(hovering && imgui_instance.scroll) {
      listView->scroll_pos -= imgui_instance.scroll / scroll_step;
  ...
  if(imgui_instance.released) listView->off = -1.f;


  
  //TEXT
  Vec2f file_pos = vec2f(pos.x, pos.y + size.y - font_height);
  size_t i = 0;
  if(fit_on_screen) {    
    i = (size_t) (listView->scroll_pos * scroll_step);
  }
  int result = -1;
