...
IMGUI_DEF int imgui_list(Vec2f pos, Vec2f size, Vec4f primary, Vec4f secondary, Imgui_ListView *listView) {
    ...
    bool fit_on_screen = size.y < (float) imgui_instance.font->height * listView->len;
    if(fit_on_screen) {
	IMGUI_RENDERER_UPDATE_SHADER(SHADER_FOR_COLOR);
	...
	renderer_solid_rect(&imgui_instance.renderer, pane_pos, pane_size, secondary);
    } else {
	listView->off = -1.f;	
    }
    if(imgui_instance.released) listView->off = -1.f;

    //cells: size.y / (float) imgui_instance.font->height
    //scroll_pos 0..(size.y - pane_size.y)
    //max_start_i: listView->len - cells;
    //i: 0 .. (listView->len - size.y / (float) imgui_instance.font->height)
    ...
    size_t i = 0;
    if(fit_on_screen) {
	i = (size_t) (listView->scroll_pos * (1 + listView->len - size.y / (float) imgui_instance.font->height) / (size.y - pane_size.y));
    }
    int result = -1;
