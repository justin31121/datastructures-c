...
YOUTUBE_DEF bool youtube_results_first(string term, Http *http, String_Buffer *sb, string *videoId);
...
YOUTUBE_DEF bool youtube_info_first_stream(string response_string, string tag, string *signature, bool *is_signature);
...
YOUTUBE_DEF bool youtube_on_elem_info_init(Json_Parse_Type type, string content, void *arg, void **elem) {
	...
	if( (string_index_of(content, "video/mp4") >= 0)  ||
		...
		} else if(string_eq(foo->prev, foo->tag) ) {
		    foo->state = 1;
		    //pass
		}
	    } else {
...
YOUTUBE_DEF bool youtube_info_first_stream(string response_string, string tag, string *signature, bool *is_signature) {
  ...
  *is_signature = info.is_signature;
...
YOUTUBE_DEF string youtube_response_match_var_declare_matches(string var_name, string response) {
  ...
  while(offset < response.len) {
    ...
    //printf("\t'%.*s'\n", (int) prefix_len, response.data+ pre_pos); fflush(stdout);
    ...
    ok = true;
    for(size_t i=offset;i<(size_t) suf_pos;i++) {
      if(!regex_alnum(response.data[i]) && response.data[i]!='$') {
	ok = false;
	break;
      }
    }
...
YOUTUBE_DEF bool youtube_decoder_init(string response_string, Http *http, String_Buffer *sb, Youtube_Decoder *decoder) {
  ...
  string var_name_match = youtube_response_match_var_declare_matches(varNameMatches_string,
								     string_from(sb->data + sb_len, jsFile_len) );
  if(!var_name_match.len) {
      printf("varNameMatches: '"String_Fmt"'\n", String_Arg(varNameMatches_string) );
