...
int main(int argc, char **argv) {
	...
	* bytes_per_pixel * frames.cap;
    ...
    if(!audio_init(&audio, 2, demuxer.audio_sample_rate)) {
	return 1;
    }
	...
	if(release) {
	    ...
	    double frame_start = (double) (av_rescale_q(frames.ptss[0], demuxer.video_time_base, AV_TIME_BASE_Q) * 1000 / AV_TIME_BASE);
	    double sample_start = frame_start;
	    if(samples.count) {
		sample_start = (double) (av_rescale_q(samples.ptss[0], demuxer.audio_time_base, AV_TIME_BASE_Q) * 1000 / AV_TIME_BASE);
	    }
