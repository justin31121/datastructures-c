...
void *audio_thread(void *arg) {
  ...
  while(!meta.stop) {
    ...
    if(audio_pos < samples->count && local_now >= audio_ms) {
...
void *decoding_audio_thread(void *arg) {
  ...
  while(!meta.stop) {
    ...
    thread_sleep(10);
    fprintf(stderr, "video_pos: %ld, Frames.count: %ld (%ld)\n", video_pos, demuxer_video.count, demuxer_video.count - video_pos); fprintf(stderr, "audio_pos: %ld, Samples.count: %ld (%ld)\n", audio_pos, meta.samples->count, meta.samples->count - audio_pos); fflush(stderr);
  }
  return NULL;
}
